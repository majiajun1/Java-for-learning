...
**Host 头处理** : HTTP/1.1 在请求头中加入了`Host`字段  域名系统（DNS）允许多个主机名绑定到同一个 IP 地址上，但是 HTTP/1.0 并没有考虑这个问题 （多了个标签）
...
3.0新增了 QUIC    UDP协议的升级版

QUIC 建立连接握手次数是两次

解决了队头阻塞问题  使用类似NIO的解决方法 多路复用数据流
...
第二次  服务端发 ACK=1 SYN=1   Ack号为K+1 序列号为n   第二次是合并了syn和ack
...
如果丢失  客户端在等第三次握手  迟迟收不到确认 就会重发syn ack （重新第二次握手）
