...
当然，除了通过我们自己SpringSecurity使用去解决之外，随着现在的浏览器不断发展，安全性越来越受到重视，很多浏览器都有**SameSite保护机制，当用户在两个不同域名的站点操作时，默认情况下Cookie就会被自动屏蔽**：
...
简单来说，就是**黑客把他的JSESSIONID直接给你，你一旦使用这个ID登录，那么在后端这个ID就被认定为已登录状态，那么也就等同于他直接进入了已登录状态，从而直接访问你账号的任意内容，执行任意操作。**
...
`UserDetailsManager` 是 Spring Security 中提供的一个接口，它扩展了 `UserDetailsService` 接口，并增加了一些用户管理相关的功能。它常用于在支持用户存储（如数据库或内存）的应用中管理用户。

### 接口定义

`UserDetailsManager` 是一个接口，定义在 `org.springframework.security.provisioning` 包中，主要用于管理用户数据。它继承了 `UserDetailsService`，除了加载用户信息外，还可以执行添加、更新、删除和检查用户存在等操作。

### 核心方法

以下是 `UserDetailsManager` 的核心方法：

#### 继承自 `UserDetailsService`

- **`loadUserByUsername(String username)`** 加载指定用户名的用户详细信息。

#### 新增的用户管理方法

- **`void createUser(UserDetails user)`** 创建一个新用户。
- **`void updateUser(UserDetails user)`** 更新一个用户的详细信息。
- **`void deleteUser(String username)`** 删除指定用户名的用户。
- **`void changePassword(String oldPassword, String newPassword)`** 更改当前已认证用户的密码。
- **`boolean userExists(String username)`** 检查指定的用户名是否存在。
...
public JSONObject changePassword(@RequestParam String oldPassword,
    ...
    JSONObject object = new JSONObject();   json默认为null
    object.put("success", true);
    return object;
}
```
...
默认情况下，**注销操作会清除用户的 `Session` 信息，但不会自动删除所有的 `Cookie`**    可以自定义配置
...
.authorizeHttpRequests(auth -> {
    ...
    //只有具有以下角色的用户才能访问路径"/"
    auth.requestMatchers("/").hasAnyRole("user", "admin");  要有这句话才行 不然连首页都进不去
    //其他所有路径必须角色为admin才能访问
    auth.anyRequest().hasRole("admin");   anyRequest()只能用一次
})
```
...
原文链接：https://www.itbaima.cn/document/63v73g0zh1qlr6fk