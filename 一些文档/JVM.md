...
JVM进行GC时 并不是对三个区域一起统一回收

99%回收在新生代

GC:轻GC  、重GC（全局GC）

新生代——伊甸园、幸存0、幸存1区 from、to
...
### 引用计数法
 ...
 每个对象计数 垃圾清楚 有点像LFU    有点Low JVM不用

问题缺点：两个对象互相引用 但是实际上没有任何作用 所以引用计数法无法回收它们
...
1、每次GC 都会将Eden活的对象移到幸存区  一旦Eden区被GC后，就会是空的

2、要保证有一个是from  有一个是to  所以要复制 保证一个比另外一个多 复制完后要清空原来的    反复横跳   把还存活的复制到另外一个，原本的空间再GC一次

3.15次GC 去养老区（可调）

好处：相比于简单的标记清除 没有内存碎片 后者有
...
标记清除缺点：内存碎片，两次扫描，浪费时间

优点：空间大（内存碎片会影响实际大小）

改进：压缩算法，再次扫描  移动对象 清除内存碎片  缺点：时间长 有移动成本
